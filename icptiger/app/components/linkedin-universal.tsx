...
import { ArkoseCaptcha } from "./arkose-captcha";
...
export function LinkedInUniversal() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.status === "success" && result.cookies) {
        ...
        console.log("🔍 CAPTCHA REQUIRED detected");
        console.log("🔍 CAPTCHA DATA:", result.captchaData);

        setCaptchaData(result.captchaData);
        setSessionId(result.sessionId); // Save sessionId for later use
        setShowCaptcha(true);
        setError(null);
        setLoginStatus("idle");
  ...
  const handleCaptchaSolved = async (token: string) => {
    ...
    try {
      ...
      // Send token back to backend to continue login process
      const response = await fetch("/api/linkedin/continue-after-captcha", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          token: token,
          captchaData: captchaData,
          sessionId: sessionId || "temp-session-id", // Use saved sessionId
        }),
      });
      ...
      if (result.status === "success" && result.cookies) {
        ...
        throw new Error(result.message || "Failed to complete login after captcha");
      }
    } catch (error) {
      console.error("❌ Error submitting captcha:", error);
      setError(error instanceof Error ? error.message : "Failed to submit captcha");
      setLoginStatus("failed");
    }
  };
  ...
  const handleCaptchaClose = () => {
    ...
    setShowCaptcha(false);
    setCaptchaData(null);
    setSessionId(null); // Clear sessionId when closing captcha
    setLoginStatus("idle");
  };
            ...
            <div className="mt-4 bg-green-50 border border-green-200 rounded-lg p-3">
              <div className="flex items-center">
                ...
                <p className="text-green-800 text-sm">
                  Login successful! Processing your account...
                </p>
              </div>
            </div>
          )}
