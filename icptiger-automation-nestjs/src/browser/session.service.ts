...
export class SessionService {
  ...
  async createSession(userId: string): Promise<BrowserSessionInfo> {
    try {
      ...
      setTimeout(
        ...
        30 * 60 * 1000, // Increased session timeout to 30 minutes
      );
  ...
  async storeExistingSession(
    userId: string,
    browser: Browser,
    page: Page,
  ): Promise<BrowserSessionInfo> {
    try {
      this.logger.log(`Storing existing browser session for user ${userId}`);
      ...
      this.logger.log(
        `Successfully stored existing session for user ${userId}`,
      );
      return sessionInfo;
    } catch (error) {
      this.logger.error(
        `Failed to store existing session for user ${userId}:`,
        error,
      );
      throw error;
    }
