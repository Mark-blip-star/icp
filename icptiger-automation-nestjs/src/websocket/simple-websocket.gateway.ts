  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
    ...
    try {
      ...
      if (hasSession) {
        ...
        if (session && session.page) {
          this.logger.log(
            `[${userId}] Session active, URL: ${session.page.url()}`,
          );
          ...
          client.emit('loginStarted', {
            message: 'LinkedIn login session ready',
            url: session.page.url(),
          });
        }
      } else {
        ...
        try {
          await this.linkedInAutomationService.runWithLogin(
            ...
            async ({ page }) => {
              ...
              this.linkedInAutomationService.setScreenshotUpdateCallback(
                ...
                async () => {
                  try {
                    ...
                    console.error(
                      `[${userId}] Error updating screenshot:`,
                      error,
                    );
                  }
                },
              ...
              client.emit('readyForLogin', {
                message:
                  'LinkedIn login page loaded, ready for user interaction',
                url: page.url(),
              });
      ...
      this.logger.error(`[${userId}] Error checking session status:`, error);
      client.emit('error', 'Failed to check session status');
    }
  }
      ...
      if (session && session.page && !session.page.isClosed()) {
        try {
          ...
          if (event.type === 'click') {
            ...
            console.log(
              `[${userId}] ✅ SUCCESS: Mouse clicked at (${event.x}, ${event.y})`,
            );
            ...
            try {
                ...
                await session.page.evaluate(
                  (x, y) => {
                    ...
                    if (
                      ...
                      console.log(
                        'Input field focused, cleared, and cursor positioned',
                      );
                    }
                  },
                  event.x,
              ...
              if (
                !elementInfo ||
                (!elementInfo.isInput && !elementInfo.isButton)
              ) {
                console.log(
                ...
                const emailField = await session.page.$(
                  'input[type="email"], input[name="session_key"], input[placeholder*="email"], input[placeholder*="Email"], input[type="text"]',
                );
                if (emailField) {
                  ...
                  if (box) {
                    ...
                    console.log(
                      `[${userId}] Found email field at (${centerX}, ${centerY})`,
                    );

                    if (
                      Math.abs(event.x - centerX) < 150 &&
                      ...
                      console.log(
                        `[${userId}] Clicked, focused, and cleared email field`,
                      );

                      // Send input field update to frontend
                      const emailFieldInfo = await session.page.evaluate(
                        (element) => {
                          return {
                            tagName: element.tagName,
                            type: (element as HTMLInputElement).type || '',
                      ...
                      console.log(
                        `[${userId}] Sent email field update to frontend:`,
                        emailFieldInfo,
                      );
                    }
                  }
                ...
                if (passwordField) {
                  ...
                  if (box) {
                    ...
                    if (
                      ...
                      console.log(
                        `[${userId}] Clicked, focused, and cleared password field`,
                      );

                      // Send input field update to frontend
                      const passwordFieldInfo = await session.page.evaluate(
                        (element) => {
                          return {
                            tagName: element.tagName,
                            type: (element as HTMLInputElement).type || '',
                      ...
                      console.log(
                        `[${userId}] Sent password field update to frontend:`,
                        passwordFieldInfo,
                      );
                    }
                  }
                ...
                const signInButton = await session.page.$(
                  'button[type="submit"], button:contains("Sign in"), button:contains("Log in")',
                );
                if (signInButton) {
  ...
  async handleKeyboardEvent(
    ...
    try {
        ...
        const activeElementInfo = await session.page.evaluate(() => {
          ...
          if (activeElement) {
            return {
              ...
              placeholder:
                (activeElement as HTMLInputElement).placeholder || '',
            };
          }
          return null;
        });
      ...
      console.log(
        `[${userId}] Could not get active element info:`,
        error.message,
      );
    }
    ...
    try {
      ...
      if (session && session.page && !session.page.isClosed()) {
        try {
          ...
          console.log(
            `[${userId}] Getting updated element info after keyboard event...`,
          );
            ...
            if (
              ...
              try {
                ...
                if (
                  ...
                  console.log(
                    'Browser: Skipping setSelectionRange for type:',
                    input.type,
                  );
                }
              } catch (error) {
                console.log(
                  'Browser: Error setting selection range:',
                  error.message,
                );
              }
              ...
              console.log(
                'Browser: Returning element info with multiple position options:',
                elementInfo,
              );
              return elementInfo;
            }
          ...
          console.log(
            `[${userId}] Position in element info:`,
            updatedElementInfo?.position,
          );
          ...
          if (updatedElementInfo) {
            ...
            console.log(
              `[${userId}] Update data with position:`,
              JSON.stringify(updateData, null, 2),
            );
            client.emit('inputUpdated', updateData);
            console.log(
              `[${userId}] ✅ SUCCESS: Sent input update to frontend:`,
              JSON.stringify(updateData, null, 2),
            );
          } else {
  ...
  async handleGetScreenshot(
    ...
    try {
      ...
      if (session && session.page) {
        this.logger.log(
          `[${userId}] Taking screenshot from page: ${session.page.url()}`,
        );
        ...
        this.logger.log(
          `[${userId}] Screenshot taken successfully, buffer size: ${screenshot.length} bytes`,
        );

        const base64Image = screenshot.toString('base64');
        this.logger.log(
          `[${userId}] Base64 conversion successful, length: ${base64Image.length} chars`,
        );

        const dataUrl = `data:image/jpeg;base64,${base64Image}`;
        this.logger.log(
          `[${userId}] Data URL created, length: ${dataUrl.length} chars`,
        );
        this.logger.log(
          `[${userId}] Data URL starts with: ${dataUrl.substring(0, 50)}...`,
        );
        ...
        try {
          ...
          this.logger.error(
            `[${userId}] ❌ Base64 validation failed:`,
            validationError,
          );
        }
        ...
        this.logger.warn(
          `[${userId}] No valid session or page found, sending default image`,
        );
        ...
        this.logger.log(
          `[${userId}] Sending default image, data length: ${dataUrl.length}`,
        );
        client.emit('screencast', screencastData);
      }
    } catch (error) {
    ...
    try {
      ...
      if (session && session.page) {
        ...
        switch (fieldType) {
          case 'email':
            selector =
              'input[type="email"], input[name="session_key"], #username, input[autocomplete="username"]';
            break;
          case 'password':
            selector =
              'input[type="password"], input[name="session_password"], #password, input[autocomplete="current-password"]';
            break;
          case 'signin':
            selector =
              'button[type="submit"], input[type="submit"], .sign-in-form__submit-button, button:contains("Sign in"), button:contains("Sign In")';
            break;
          default:
            selector = fieldType;
        }
        ...
        try {
          ...
          this.logger.log(
            `[${userId}] ✅ Clicked field by selector: ${selector}`,
          );
          clicked = true;
        } catch (selectorError) {
          this.logger.warn(
            `[${userId}] Selector click failed: ${selectorError.message}`,
          );
        }
        ...
        if (!clicked && fieldType === 'signin') {
          try {
            ...
            if (buttonText) {
              ...
              this.logger.log(
                `[${userId}] ✅ Clicked sign in button by text: ${buttonText}`,
              );
              clicked = true;
            }
          } catch (textError) {
            this.logger.warn(
              `[${userId}] Text-based click failed: ${textError.message}`,
            );
          }
        }
        ...
        if (!clicked) {
          throw new Error(
            `Failed to click ${fieldType} field using all methods`,
          );
        }
  ...
  private async startCDPScreencast(
    ...
    try {
      const session =
        await this.linkedInAutomationService.getSessionInfo(userId);
      const cdpSession =
        await this.linkedInAutomationService.getCDPSession(userId);

      if (!session || !cdpSession) {
        this.logger.warn(
          `[${userId}] No session or CDP session available for screencast`,
        );
        return;
      }
      ...
      cdpSession.on('Page.screencastFrame', async (data) => {
        try {
          ...
          console.log(
            `[${userId}] CDP screencast frame received but skipped to prevent [object Object] issue`,
          );
          return;
        } catch (error) {
          this.logger.error(
            `[${userId}] Error in CDP screencast frame:`,
            error,
          );
        }
      });
      ...
      this.logger.log(
        `[${userId}] CDP screencast started successfully (disabled for manual updates)`,
      );
    } catch (error) {
  ...
  private async sendInputUpdate(
    ...
    try {
      ...
      if (activeElementInfo) {
        ...
        client.emit('inputUpdated', updateData);
        console.log(
          `[${userId}] Sent input update via sendInputUpdate:`,
          JSON.stringify(updateData, null, 2),
        );
      } else {
        ...
        client.emit('inputUpdated', updateData);
        console.log(
          `[${userId}] Sent empty input update to clear overlay:`,
          JSON.stringify(updateData, null, 2),
        );
      }
    } catch (error) {
  ...
  private async forceScreenshotUpdate(
    userId: string,
    client: Socket,
    page: any,
  ): Promise<void> {
    try {
      ...
      console.log(
        `[${userId}] Screenshot taken, size: ${base64Image.length} chars`,
      );
      console.log(
        `[${userId}] Data URL starts with: ${dataUrl.substring(0, 50)}...`,
      );
